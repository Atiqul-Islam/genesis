...
def main():
    # argv: <expertise_dir> [<agent>] [--main-agent <name>]. The built-agent frontmatter path (assemble.py)
    # and the tests pass <agent> explicitly; the plugin SessionStart/SubagentStart hooks omit it and instead
    # carry the active agent in the payload's `agent_type` (with --main-agent sensei as the main-thread fallback).
    main_agent, pos = "", []
    args = sys.argv[1:]
    i = 0
    while i < len(args):
        if args[i] == "--main-agent" and i + 1 < len(args):
            main_agent = args[i + 1]
            i += 2
            continue
        pos.append(args[i])
        i += 1
    exp_dir = pos[0] if pos else ""
    argv_agent = pos[1] if len(pos) > 1 else ""
    try:
