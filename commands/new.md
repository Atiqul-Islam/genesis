...
2. Hand Sensei the user's request: **$ARGUMENTS**
   - If that is empty, Sensei must interview the user first — goal, done-criteria, constraints, the exact tools the agent needs, escalation triggers, and (Sensei always asks, never decides) whether this is a single agent or a supervisor-led team, and whether it installs as a **subagent** (default) or the folder's **main Claude**.
   - Sensei can also **promote an existing built agent** into the folder's main Claude — same full enforcement — via `genesis-cli promote <name>`.
