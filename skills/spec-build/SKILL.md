...
6. Write initial `state.json`:
   ...
   ```
7. **Spawn all five agents** via parallel `Task` calls (single message, multiple tool uses). Each Task uses `subagent_type: "general-purpose"` and the prompt instructs the agent to load `${CLAUDE_PLUGIN_ROOT}/skills/<agent-name>/SKILL.md` as its persona and respond with `{ "status": "ready" }`. Capture each spawned agent's identifier so you can later use `SendMessage` to talk to them. **Agents remain alive for the entire workflow.**
...
- The five agent persona files: `${CLAUDE_PLUGIN_ROOT}/skills/{spec,dev,verify,review,docs}-agent/SKILL.md`
