...
Genesis's fetch-launcher (`bin/genesis-memory.js`) and repo test tooling
(`test/tools/*.mjs`) are implemented in **Node.js** and run on Node's built-in
standard library alone. They declare and require **no third-party runtime
dependencies** and are covered by Genesis's own MIT license. Running the launcher
requires Node.js (alongside Claude Code); it does **not** require Python. Everything
else — the enforcement hooks, the installer/orchestrator, and the session-copy
pipeline — is a Rust binary (`genesis-hook`, `genesis-cli`), which the launcher stages
and execs — see §1–§4.
