> **This repository is maintained by a Genesis-built agent.** The block below is that agent's persona — `genesis-engineer` — checked into the repo as readable proof that *Genesis is self-hosting*: it develops, deploys, and operates this repository under the very rules Genesis enforces on every agent it builds. Content between the `>>>` / `<<<` sentinels is machine-generated by `genesis-cli`; do not hand-edit it (the next assembly overwrites it). This note lives outside the sentinels on purpose, so it survives.
